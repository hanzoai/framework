...
func TestIngestValidatesAndFiresHooks(t *testing.T) {
	...
	fired := 0
	resetHooks()
	RegisterHook(at("test", "note"), ActionAfterSave, func(_ context.Context, ev *Event) error {
		fired++
		gotOrg = ev.Org
		gotName = ev.Doc.Name
	...
	ing, err := s.Ingest(context.Background(), "acme", at("test", "note"), map[string]any{
	...
	doc, err := s.store.GetDocument(context.Background(), "acme", at("test", "note"), ing.Name)
	if err != nil {
	...
	if _, err := s.store.GetDocument(context.Background(), "other", at("test", "note"), ing.Name); err == nil {
...
func TestIngestRejectsMissingRequired(t *testing.T) {
	...
	if _, err := s.Ingest(context.Background(), "acme", at("test", "note"), map[string]any{"body": "x"}, ""); err == nil {
...
func TestIngestUnknownDocTypeErrors(t *testing.T) {
	s, done := mountForIngest(t)
	defer done()
	if _, err := s.Ingest(context.Background(), "acme", at("test", "does-not-exist"), map[string]any{}, ""); err == nil {
...
func TestDeleteRunsTrashHookAndRemoves(t *testing.T) {
	...
	ing, err := s.Ingest(context.Background(), "acme", at("test", "note"), map[string]any{"title": "x"}, "")
	if err != nil {
		t.Fatalf("Ingest: %v", err)
	}
	...
	fired := 0
	resetHooks()
	RegisterHook(at("test", "note"), ActionOnTrash, func(_ context.Context, _ *Event) error { fired++; return nil })
	t.Cleanup(resetHooks)
	if err := s.Delete(context.Background(), "acme", at("test", "note"), ing.Name); err != nil {
		t.Fatalf("Delete: %v", err)
	}
	if fired != 1 {
		t.Fatalf("on_trash fired %d times, want 1", fired)
	}
	if _, err := s.store.GetDocument(context.Background(), "acme", at("test", "note"), ing.Name); err == nil {
		t.Error("document still present after Delete")
	}

	// A missing document is ErrNotFound (idempotent from the caller's view).
	if err := s.Delete(context.Background(), "acme", at("test", "note"), "nope"); err != ErrNotFound {
		t.Errorf("Delete of missing doc = %v, want ErrNotFound", err)
	}

	// A gate error aborts the delete.
	ing2, _ := s.Ingest(context.Background(), "acme", at("test", "note"), map[string]any{"title": "y"}, "")
	resetHooks()
	RegisterHook(at("test", "note"), ActionOnTrash, func(_ context.Context, _ *Event) error { return context.Canceled })
	if err := s.Delete(context.Background(), "acme", at("test", "note"), ing2.Name); err == nil {
		t.Error("Delete must surface an on_trash gate error")
	}
	if _, err := s.store.GetDocument(context.Background(), "acme", at("test", "note"), ing2.Name); err != nil {
